...
exports.AddFormConfigurationMsq = async (data, Callback) => {
    ...
    if(data.list.length){
        let addSqlParams = data.list.map(item => {
            return [item.id?item.id:0, item.formId, item.formModel, item.label, item.type, item.size, item.isCheck, item.editlist, item.disabled, item.isValidator, JSON.stringify(item.rules), item.btnFun, item.btnType, item.text,item.sortId,item.newDate,item.selectCustom,item.dictionaryKey,item.dicDefault,item.occupiedColumns,item.placeholder]
        })
        let sql = `insert into formConfiguration  (id, formId, formModel,label, type, size, isCheck, editlist, disabled, isValidator, rules, btnFun, btnType, text, sortId, newDate, selectCustom, dictionaryKey, dicDefault,occupiedColumns,placeholder) values ? ON DUPLICATE KEY UPDATE formModel= values(formModel),label= values(label),type=values(type),size=values(size),isCheck=values(isCheck),editlist=values(editlist), disabled=values(disabled), isValidator=values(isValidator), rules=values(rules), btnFun=values(btnFun), btnType=values(btnType), text=values(text), sortId=values(sortId), newDate=values(newDate),selectCustom=values(selectCustom),dictionaryKey=values(dictionaryKey),dicDefault=values(dicDefault),occupiedColumns=values(occupiedColumns),placeholder=values(placeholder)`;
